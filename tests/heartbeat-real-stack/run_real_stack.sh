#!/usr/bin/env bash
# Real-stack E2E harness for the claude-code-plugin Community-SaaS-default
# rollout (ADR-048).
#
...
if [ -f "$WORK_DIR/_pings.jsonl" ]; then
  ...
  # registration file's tenant_id. In this harness the registration
  # cycle ran above and stored a cs_<uuid> at REG_FILE — assert the
  # captured ping body carries that exact value.
...
STAMP_FILE="${SANDBOX_HOME}/.cache/axonflow/claude-code-plugin-telemetry-sent"
