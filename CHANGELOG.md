...
- **The post-tool hook no longer drops an output deny.** It read `allowed` with `.allowed // true`, which jq evaluates to true for `false`; it now reads the boolean explicitly.
...
- **Both attach points carry it, and a test proves they agree.** This plugin builds headers in two places: the `headersHelper` inside `.mcp.json` for the MCP session, and `scripts/` for every hook call. A declaration added to one and not the other would be absent from half the plugin's traffic, silently. `tests/test-pep-handshake.sh` asserts that both produce the *same* bytes and that those bytes are the platform's own.
- **Off by default, and absent rather than empty when off.** Leave `AXONFLOW_PEP_AUDIENCE` unset and no header is sent at all, on either carrier, so the plugin behaves byte for byte as 1.11.0 against every platform version. The distinction is load bearing: a header present with an empty value is malformed to the platform and would refuse the call, which an absent header does not.
...
## [1.11.0] - 2026-07-18 — caller_name audit attribution (dual-send with legacy tool_type)

### Changed

- **`audit_tool_call` now sends `caller_name` to identify the calling plugin, dual-sent with the legacy `tool_type` for the deprecation window** (#110; axonflow-enterprise#2912, sub-issue of epic #2905). `tool_type` was misleadingly named — it was never a tool classification, it identified WHICH CLIENT made the call. The platform side (axonflow-enterprise#2953, shipped in platform 9.11.0) adds the correctly-named `caller_name` field for this purpose and resolves `caller_name → tool_type → default`, writing only `caller_name` on new rows; `tool_type` remains a deprecated legacy fallback that pre-#2953 platforms still honor. Every `audit_tool_call` send site in this plugin — `scripts/post-tool-audit.sh` (PostToolUse) and `scripts/pre-tool-check.sh` (the blocked/redacted PreToolUse audit POSTs) — now sends **both** `caller_name: "claude_code"` and `tool_type: "claude_code"`. Dual-send makes attribution exact on a 9.11.0+ platform (where `caller_name` wins) **and** status-quo on any pre-#2953 platform (where the legacy `tool_type` still attributes the row), so the plugin can ship on the marketplace ahead of a customer's platform upgrade without a silent attribution regression. `tool_type` will be dropped in a later release once the platform floor includes #2953. This mirrors the same transition pattern across the codex/cursor/openclaw plugins so all four sibling plugins stay coherent. The host-CLI shim (`tests/host-cli-shim/run.sh`, CI-gated) now asserts the on-the-wire `audit_tool_call` payload carries both keys, so a silent revert fails CI rather than passing on request-count alone.
...
- **`scripts/user-token.sh` now honors `AXONFLOW_CONFIG_DIR`** (the #108 drift class on the directory axis, caught in the same review): the inline headersHelper, `self-hosted-auth.sh`, and `status.sh` all resolve the config dir as `${AXONFLOW_CONFIG_DIR:-$HOME/.config/axonflow}`, but the resolver hardcoded `$HOME/.config/axonflow` — a fleet that relocates the config dir got the token on the MCP plane but not the hooks. Same 0600 discipline in the relocated dir; pinned on both resolvers (`tests/test-user-token.sh` leg 6c + `tests/test-mcp-headers.sh` leg 30).

- **Every config-file consumer now honors `AXONFLOW_CONFIG_DIR`** (the #108 drift class on the remaining credential axes, caught in the #109 follow-up review). `license-token.sh` (the hook-plane X-License-Token reader AND the dir `login.sh` persists into), `recover-verify.sh`, `community-saas-bootstrap.sh`, and `telemetry-ping.sh` hardcoded `$HOME/.config/axonflow` while the inline `.mcp.json` headersHelper, `self-hosted-auth.sh`, `status.sh`, and `user-token.sh` resolve `${AXONFLOW_CONFIG_DIR:-$HOME/.config/axonflow}` — a relocated fleet got the license token on the MCP plane but not the hooks, and `/axonflow-login` wrote a token nothing read. Pinned by a config-dir leg in `tests/test-license-token-cache-skip.sh`.
