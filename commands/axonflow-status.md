...
- `tier=Free (Pro expired 2026-02-04 — visit https://www.getaxonflow.com/pricing/ to renew)` — token configured but its `exp` is in the past; plugin will not forward it. User must renew + re-run `/axonflow-login --token <new>`.
